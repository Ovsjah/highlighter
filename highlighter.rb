...
class Highlighter
  ...
  private
    ...
    def write_to_file
      File.write('./highlighted.html', highlighted)
    end
    ...
    def words
      @words ||= paragraphs.join(' ').split(' ')
    end

    def highlighted
      @highlighted ||=
        highlights.each_with_object(words.dup) do |to_highlight, copy|
    ...
    def highlight(words_array, comment)
      regex = /(?<start>(?<s>^(?!<p>))?(?(<s>)^|(?<=<p>)))|(?<end>(?<e>(?<!<\/p>)$)?(?(<e>)$|(?=<\/p>)))/
        ...
        if $~[:start]
          "<span title='#{comment}'"\
              " style='cursor: pointer;"\
                     ...
                     " background-color: #{colorize(string)}'>"
        elsif $~[:end]
          '</span>'
        end
      end.split('&nbsp;')
...
Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Integer lacinia accumsan velit. Duis vel facilisis libero. Cras consequat sit amet mauris ut ultrices. Ut pulvinar sit amet odio sit amet pretium. Nullam tortor ligula, consequat non nisl vitae, rutrum placerat est. Sed finibus interdum justo vel placerat. Cras varius tortor sed justo tempus scelerisque. Praesent facilisis ex vitae iaculis iaculis. Sed consectetur a lectus non condimentum. Etiam id lacus a nulla cursus laoreet. Vivamus ipsum purus, sodales vel metus varius, viverra mollis justo. Nulla facilisi. Vivamus volutpat nunc elit, quis sollicitudin velit ornare sit amet towelie.
...
p Highlighter.new(content, highlights).apply
